...
public class SF03StudentBeanController {
	...
	public ResponseEntity<SF03StudentBean> updateStudent(@PathVariable Long id,
			@Validated @RequestBody SF03StudentBean student) {
		return ResponseEntity.ok(studentService.updateStudent(id, student));
	}

	@PatchMapping(path = "api/v1/updateStudentsPartially/{id}")
	public ResponseEntity<SF03StudentBean> updateStudentPartially(@PathVariable Long id,
			@Validated @RequestBody SF03StudentBean student) {
		return ResponseEntity.ok(studentService.updateStPartially(id, student));
	}

	@PostMapping(path = "api/v1/addNewStudent")
	public ResponseEntity<SF03StudentBean> updateStudentPartially(@Validated @RequestBody SF03StudentBean student)
